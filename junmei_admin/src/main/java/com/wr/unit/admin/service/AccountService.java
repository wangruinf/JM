...
public class AccountService {
    ...
    public User registerUser( User user) throws RegistrationException{
        ...
        Assert.notNull(user , "Param User entity is not null!");

        User tmpUser = userDao.findByLoginName( user.getLoginName() );
        if( tmpUser != null ) throw new RegistrationException("用户帐号已经在!");
     ...
     * 按页面传来的查询条件查询用户.
     */
    public Page<User> searchUserByPage( Map<String, Object> searchParams, PageRequest pageRequest) {
        Map<String, SearchFilter> filters = SearchFilter.parse(searchParams);
        Specification<User> spec = DynamicSpecifications.bySearchFilter(filters.values(), User.class);
...
/**
    ...
    public Team savaTeam(Team team){
    ...
    public Page<Team> searchTeamByPage( Map<String, Object> searchParams, PageRequest pageRequest) {
        Map<String, SearchFilter> filters = SearchFilter.parse(searchParams);
        Specification<Team> spec = DynamicSpecifications.bySearchFilter(filters.values(), Team.class);
        Page<Team> page = teamDao.findAll(spec, pageRequest);
        return page;
    }
	// -----------------//
	// Setter methods //
